...
class StuckbucksCaptivePortalChecker(object):
    """
        StuckbucksCaptivePortalChecker is a tool for checking the process of captive portal in stuckbucks
        Since the implementation of captive portal in stuckbucks will be changed in the future
        ...
        if pkt.haslayer(TCP):
            try:
                record = {
                    ...
                    'protocol': 'HTTP',
                    'time': pkt.time,
                    'ret_code': StuckbucksCaptivePortalChecker.parse_raw_load(codecs.decode(pkt[Raw].load, 'utf-8')),
                    'dst': pkt[IP].dst
                }
    ...
    def tls_capture(self, pkt, cnt):
        ...
        if pkt.haslayer(TCP) and pkt[TCP].dport == 443:
            try:
                tcp_load = StuckbucksCaptivePortalChecker.parse_tcp_load(bytes(pkt[TCP].load))
                record = {
                    'cnt': cnt,
                    'protocol': 'HTTPS',
                    'time': pkt.time,
    ...
    def get_tls_content_type(content_type_code):
        ...
        return StuckbucksCaptivePortalChecker.__get_type(content_type_code, tls_content_type)
    ...
    def get_handshake_type(handshake_type_code):
        ...
        return StuckbucksCaptivePortalChecker.__get_type(handshake_type_code, handshake_type)
    ...
    def parse_tcp_load(tcp_load):
        ...
        ret = {
            ...
            'content_type': StuckbucksCaptivePortalChecker.get_tls_content_type(tcp_load[0]),
            'handshake_type': StuckbucksCaptivePortalChecker.get_handshake_type(tcp_load[5])
        }
        return ret
